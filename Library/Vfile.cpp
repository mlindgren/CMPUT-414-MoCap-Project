...
void Datagroup::print()
{
  if (GroupID==-1)
  {
    cout << "Datagroup not inited" << endl;
    return;
  }
...
void Vfile::read_header(ifstream &v)
{
  ...
  if (buffer[0]!='V')
  {
    cout << "ERROR: expected V, got " << buffer[0] << endl;
    read_ok = false;
  }
  if (buffer[1]!='#')
  {
    cout << "ERROR: expected #, got " << buffer[1] << endl;
    read_ok = false;
  }
  readType(v, &version);
...
void Vfile::read_dynamics(ifstream &v)
{
  ...
  while(1)
  {
    ...
    if (v.eof())
    {
      cout << "JUST KIDDING: Actually end of v file" << endl;
      return;
    }
