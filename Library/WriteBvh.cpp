...
Vector3d vecd(const Vector3f &f)
{
  Vector3d d;
  d.x=f.x;
  d.y=f.y;
...
void write_self_and_kids(ostream &os, const Library::Skeleton &skel, int self, int num_tabs, vector< int > & order)
{
  ...
  Vector3d direction;
  float length;
  if (skel.bones[self].parent==-1)
  {
    direction = make_vector<double, 3>(0.0);
    length = 0;
  }
  else
  {
    direction = skel.bones[skel.bones[self].parent].direction;
    length = skel.bones[skel.bones[self].parent].length;
  }
