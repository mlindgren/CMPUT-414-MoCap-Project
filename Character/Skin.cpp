...
void Skin::set_skin_mesh(InputMesh const &mesh)
{
  ...
  for (VertMap::iterator vert = used.begin(); vert != used.end(); ++vert)
  {
    ...
    {
      *store = mesh.colors[vert->first.color].c[j];
      ++store;
    }
    for (unsigned int j = 0; j < 3; ++j)
    {
      *store = mesh.norms[vert->first.norm].c[j];
      ++store;
    }
    for (unsigned int j = 0; j < 4; ++j)
    {
      *store = mesh.verts[vert->first.vert].weights.c[j];
      ++store;
    }
    for (unsigned int j = 0; j < 4; ++j)
    {
      assert(mesh.verts[vert->first.vert].bones.c[j] >= -1);
      *store = (float)(mesh.verts[vert->first.vert].bones.c[j] + 1);
      ++store;
    ...
    for (unsigned int j = 0; j < 3; ++j)
    {
      *store = mesh.verts[vert->first.vert].pos.c[j];
      ++store;
    }
  }
  ...
  for (unsigned int t = 0; t < mesh.triangles.size(); ++t)
  {
    ...
    for (unsigned int i = 0; i < 3; ++i)
    {
      ...
      else
      {
        ...
        *index = vert_count + (i + 1) % 3;
        ++index;
        *index = vert_count + i;
        ++index;
        *index = f->second.first;
        ++index;

        *index = f->second.second;
        ++index;
        *index = f->second.first;
        ++index;
        *index = vert_count + i;
    ...
    for (unsigned int i = 0; i < 3; ++i)
    {
      ...
      for (unsigned int j = 0; j < 4; ++j)
      {
        *buffer = mesh.verts[tri.c[i]].weights.c[j];
        ++buffer;
      }
      for (unsigned int j = 0; j < 4; ++j)
      {
        assert(mesh.verts[tri.c[i]].bones.c[j] >= -1);
        *buffer = (float)(mesh.verts[tri.c[i]].bones.c[j] + 1);
        ++buffer;
      ...
      for (unsigned int j = 0; j < 3; ++j)
      {
        ...
        *buffer = mesh.verts[tri.c[i]].pos.c[j];
        ++buffer;
      }
      ++vert_count;
    }
